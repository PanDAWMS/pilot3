...
class ESProcessFineGrainedProc(threading.Thread):
    ...
    def get_corecount(self):
        try:
            ...
            logger.warn("RUBIN_ES_CORES is not defined correctly: %s" % str(ex))
    ...
    def init_logs(self):
        ...
        logger.info("stdout_file: %s; stderr_file: %s" % (self.stdout_file, self.stderr_file))
        ...
        for realtime_log_file in realtime_log_files:
            ...
            logger.info("realtime log %s: %s" % (realtime_log_file, self.realtime_log_files[realtime_log_file]))
        logger.info("self.realtime_log_queues: %s" % str(self.realtime_log_queues))
    ...
    def close_logs(self):
        try:
            ...
            logger.error("Failed to close logs: %s" % str(ex))
    ...
    def decode_base64(self, sb):
        try:
            ...
            logger.error("decode_base64 %s: %s" % (sb, ex))
            return sb
    ...
    def encode_base64(self, sb):
        try:
            ...
            logger.error("encode_base64 %s: %s" % (sb, ex))
            return sb
    ...
    def get_event_executable(self, event_dir, event):
        ...
        stdout_file = open(stdout_filename, 'a')
        stderr_file = open(stderr_filename, 'a')
        realtime_log_files = os.environ.get('REALTIME_LOGFILES', None)
        realtime_log_files = re.split('[:,]', realtime_log_files)
        realtime_log_files = [os.path.join(event_dir, f) for f in realtime_log_files]
    ...
    def run_event(self, event):
        ...
        log_prefix = "worker_id=%s: " % worker_id
        try:
            event_range_id = event['eventRangeID']
            logger.info(log_prefix + "start to run event " + str(event_range_id))
            ...
            try:
                ...
                raise Exception(f'could not execute: {error}')
        ...
        logger.info(log_prefix + "ret: " + str(ret))

        time_used = time.time() - time_start
        logger.info(log_prefix + "time used to process this event: " + str(time_used))
        ...
        if self.event_execution_time is None or self.event_execution_time < time_used:
            ...
            logger.info(log_prefix + "max event execution time: " + str(time_used))
        return ret
        ...
        if self.__thread_pool:
            ...
            if not terminate:
                ...
                if num_free_workers > 0:
                    ...
                    if events:
                        logger.info("Got %s events: %s" % (len(events), events))
                    for event in events:
            ...
            if outputs:
                logger.info("Got %s outputs: %s" % (len(outputs), outputs))
    ...
    def poll(self):
        ...
        #     return None
        logger.debug("is_alive: %s, ret_code:%s" % (self.is_alive(), self.__ret_code))
        # if self.is_alive():
        #     return None
